def general_fields(row, wb, valve_type):
    # Determinar los nombres de las hojas
    tags = wb.sheetnames
    ...
    if row['TAG'] not in tags:
      ...
      tag_coincidences = len([1 for tag in tags if tag.startswith(row['TAG'])])
      sheet_name = f'{row["TAG"]} ({tag_coincidences})'       
    
    # Copiar la hoja desde el template
    old_sheet = wb[valve_type]
    sheet = wb.copy_worksheet(old_sheet)

    # renombrando la hoja nueva
    sheet.title = sheet_name   

    
    # LLENANDO CONDICIONES GENERALES COMÚN A TODAS LAS VÁLVULAS
    sheet['H3'] = row['TAG']
    sheet['H4'] = row['SIZES']
    sheet['H5'] = row['SPEC']
    sheet['H6'] = row['SERVICE']

    # LLENANDO LA PARTE MECÁNICA QUE S COMÚN A TODAS LAS VÁLVULAS
    sheet['H10'] = row['TYPE']
    ...
    sheet['AD12'] = row['CONECTION'] if row['CONECTION'] not in ['RF', 'FF', 'RTJ', 'SW', 'SCREWED', 'BUTWELD'] else ''
    sheet['Q14'] = 'X' if row['END_CODE'] == 'ASME B16.5' else ''
    sheet['Y14'] = 'X' if row['END_CODE'] == 'ASME B16.11' else ''
    sheet['AF14'] = row['END_CODE'] if row['END_CODE'] not in ['ASME B16.5', 'ASME B16.11'] else ''    
    sheet['H17'] = row['OPERATOR'] if row['OPERATOR'] != '-' else ''

    # DESIGN COMUNES A TODAS LAS VÁLVULAS
    ...
    sheet['AG22'] = row['DESIGN_CODE'] if row['DESIGN_CODE'] not in ['API 600', 'API 6D', 'API 602', 'API 608'] else ''
