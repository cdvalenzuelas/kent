...
def locking_device(mto_df, pid_df):
    # Eliminar las filas que no tienen válvulas o que las válvulas que no se hayan encontrado
    pid_df = pid_df[(pid_df['QTY'] > 0) & (pid_df['TAG'] != 'error')]

    # Eliminar las filas que no tienen cantidad o que no tienen locking device (no son relevantes para el análisis)
    ...
    # Iterar todas las filas en busca de las válvulas que tienen locking device
    for index, row in mto_df.iterrows():
        if row['NOTE'] == 'delete':
            row_dict = row

            # Extraer las cantidades de CSO, CSC y NO_LOCKING_DEVICE
            cso = row_dict['CSO']
            csc = row_dict['CSC']
            no_locking_device = row_dict['NO_LOCKING_DEVICE']

            # Creando los diccionarios necesrio para crear los dataframes que se anezarán
            if cso:
    ...
    # Eliminar las columnas a las que ya se les ha hecho un análisis de locking device
    mto_df = mto_df[(mto_df['NOTE'] != 'delete')]

    # Reconstruir el index
